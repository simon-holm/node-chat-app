...
io.on('connection', socket => {
  ...
  socket.emit(
    'newMessage',
    generateMessage('#Epic Chat', 'Bot', 'Welcome to the chat!')
  )

  // socket.broadcast.emit from Adming text = New user joined.
  socket.broadcast.emit(
    'newMessage',
    generateMessage('#Epic Chat', 'Bot', 'New user joined the chat')
  )
  ...
  socket.on('createLocationMessage', ({ chatName, user, lat, lng }) => {
    ...
    io.emit(
      'newLocationMessage',
      generateLocationMessage(chatName, user, 'Bot', lat, lng)
    )
  })
  ...
  socket.on('disconnect', () => {
    ...
    io.emit(
      'newMessage',
      generateMessage('#Epic Chat', 'Bot', 'User left the chat')
    )
  })
})
